...
class LiveTipManager(models.Manager):
    def get_query_set(self):
        return super(LiveTipManager, self).get_query_set().filter(status=self.model.LIVE_STATUS)


class Tip(SeoModel, models.Model):
    LIVE_STATUS = 1
    DRAFT_STATUS = 2
    HIDDEN_STATUS = 3
    STATUS_CHOICES = (
    ...
    def get_absolute_url(self):
        return ('tip_detail', (), {'slug': self.slug, })
...
class Block(Pos_Act, models.Model):
    ...
    article = models.ForeignKey(Tip, related_name='article_block', verbose_name=u'Статья')
